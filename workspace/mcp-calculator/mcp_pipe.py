...
async def connect_to_server(uri, target):
    ...
    try:
        ...
        async with websockets.connect(uri) as websocket:
            ...
            # Proactively initialize and list tools to complete handshake with Moss
            initialize_req = '{"jsonrpc":"2.0", "id": -1, "method": "initialize", "params": {"client_name": "mcp-pipe"}}\n'
            list_tools_req = '{"jsonrpc":"2.0","id":0,"method":"tools/list"}\n'
            
            logger.info(f"[{target}] Proactively sending initialize request to local server.")
            process.stdin.write(initialize_req)
            process.stdin.flush()
            
            await asyncio.sleep(0.2) # Give the server a moment to process initialization
            
            logger.info(f"[{target}] Proactively sending listTools request to local server.")
            process.stdin.write(list_tools_req)
            process.stdin.flush()
