...
def _get_instance(account_id: str):
    ...
    websocket = getattr(instance, "ws", None)
...
LEGACY_MARKET_CONTACT_TEMPLATE = "你好，看到你这台${item_title}还在，请问真实成色和电池情况方便再发我确认下吗？"


def _clean_market_contact_title(title: str, max_length: int = 18) -> str:
...
def _infer_market_contact_scene(item: MarketSellerContactItem) -> str:
    text = " ".join(
        part for part in [
            str(item.title or ""),
    ...
    digital_keywords = (
        ...
        "手机", "平板", "笔记本", "电脑", "相机", "耳机", "手表", "手环",
        "华为", "小米", "三星", "荣耀", "oppo", "vivo", "一加",
    )
    home_keywords = (
        "按摩椅", "沙发", "冰箱", "洗衣机", "空调", "电视", "投影", "办公椅",
        "桌子", "椅子", "床", "柜", "跑步机", "椭圆机", "家电", "家具",
        "烘干机", "蒸烤箱", "烤箱", "显示器",
    )
...
def _choose_market_contact_variant(item: MarketSellerContactItem, variants: list[str]) -> str:
    if not variants:
        return ""
    seed_text = f"{item.item_id}:{item.seller_user_id}:{item.title}"
    seed = sum(ord(char) for char in seed_text)
    return variants[seed % len(variants)]


def _build_adaptive_market_contact_message(item: MarketSellerContactItem) -> str:
    scene = _infer_market_contact_scene(item)
    short_title = _clean_market_contact_title(item.title)

    if scene == "digital":
        variants = [
            "你好，这个还在吗？想确认下真实成色、功能和电池情况",
            "你好，还在的话方便说下成色、功能和电池情况吗",
            "你好，这个还在吗？我想了解下成色、功能和电池",
        ]
    elif scene == "home":
        variants = [
            "你好，这个还在吗？想确认下真实成色和功能使用情况",
            "你好，还在的话方便说下成色和功能都正常吗",
            "你好，这个还在吗？想问下实际成色和使用情况",
        ]
    else:
        variants = [
            "你好，这个还在吗？想确认下真实成色和使用情况",
            "你好，还在的话方便说下实际成色和使用情况吗",
            "你好，这个还在吗？想简单了解下成色和使用情况",
        ]

    message = _choose_market_contact_variant(item, variants)
    if short_title and scene == "generic" and len(short_title) <= 10:
        return f"你好，{short_title}还在吗？想确认下真实成色和使用情况"
    return message


def _should_use_adaptive_market_contact_message(template_text: str) -> bool:
    normalized = str(template_text or "").strip()
    if not normalized:
        return True
    return normalized == LEGACY_MARKET_CONTACT_TEMPLATE


def _render_market_contact_message(template_text: str, item: MarketSellerContactItem) -> str:
    if _should_use_adaptive_market_contact_message(template_text):
        return _build_adaptive_market_contact_message(item)
    ...
    rendered = template.safe_substitute(
        ...
        item_title=_clean_market_contact_title(item.title) or item.item_id,
        item_title_raw=item.title or item.item_id,
...
def _get_market_contact_retry_delay(error: Exception | str, attempt: int) -> float:
    message = str(error or "").strip()
    attempt = max(1, int(attempt or 1))

    if any(marker in message for marker in ("闲鱼走神了", "RGV587", "系统繁忙", "稍后再试")):
        return min(75.0, 12.0 * attempt + 8.0 * (attempt - 1))
    if "当前聊天连接未就绪" in message:
        return min(20.0, 4.0 * attempt)
    return min(30.0, 6.0 * attempt)
...
async def _send_market_contact_message(instance, item: MarketSellerContactItem, message_text: str) -> tuple[str, str]:
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            wait_seconds = _get_market_contact_retry_delay(exc, attempt)
            logger.warning(
...
async def market_research(body: MarketResearchRequest):
    ...
    try:
        ...
        logger.info(
            ...
            f"max_pages={body.max_pages}, min_price={body.min_price}, max_price={body.max_price}, "
            f"include={body.include_terms}, exclude={body.exclude_terms}"
        )
...
async def contact_quality_sellers(body: MarketSellerContactRequest):
    ...
    try:
        ...
        if body.async_mode:
            ...
            delay_seconds = max(5.0, min(float(body.delay_seconds or 25.0), 60.0))
            asyncio.create_task(
