...
const LEGACY_CONTACT_TEMPLATE = '你好，看到你这台${item_title}还在，请问真实成色和电池情况方便再发我确认下吗？';
...
const normalizeCachedQualityForm = (
  value: Partial<typeof defaultQualityForm> | undefined,
) => {
  const merged = { ...defaultQualityForm, ...(value || {}) };
  const rawTemplate = String(value?.message_template ?? '').trim();
  const isLegacyTemplate = rawTemplate === LEGACY_CONTACT_TEMPLATE;

  if (isLegacyTemplate) {
    merged.message_template = '';
    if (Number(value?.delay_seconds || 0) <= 2) {
      merged.delay_seconds = defaultQualityForm.delay_seconds;
    }
    if (Number(value?.max_contact || 0) === 3) {
      merged.max_contact = defaultQualityForm.max_contact;
    }
  }
...
const MarketResearch: React.FC = () => {
  ...
  const [qualityForm, setQualityForm] = useState(() => normalizeCachedQualityForm(cachedState?.qualityForm));
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setQualityForm((prev) => ({ ...prev, delay_seconds: Number(e.target.value) || 25 }))}
              className="w-full ios-input px-4 py-3 rounded-xl"
            />
          </div>
        </div>
