...
import { archiveExportBundle, buildExportBundle, writeExportBundle } from "./runtime/export-bundle";
...
async function runCommand(
  ...
  if (command === "backup-export") {
    try {
      ...
      const archived = await archiveExportBundle(written);
      await appendOperationAudit({
        action: "backup_export",
        source: "command",
        ok: true,
        requestId: "cmd-backup-export",
        detail: `wrote ${written.fileName} and ${archived.fileName}`,
        metadata: {
          path: written.path,
          sizeBytes: written.sizeBytes,
          archivePath: archived.path,
          archiveSizeBytes: archived.sizeBytes,
          archiveFileName: archived.fileName,
        },
      });
