...
function getOrbitLimits(preset) {
  const azimuthCenter = Math.atan2(
...
function OfficeShell({ theme }) {
  ...
  return (
    <group>
      ...
      <Text
        position={[0, 4.62, BACK_GLASS_Z + 0.18]}
        fontSize={0.34}
        color={signColor}
        anchorX="center"
        anchorY="middle"
      ...
      <Text
        position={[0, 4.22, BACK_GLASS_Z + 0.18]}
        fontSize={0.12}
    ...
    <group position={position} scale={AVATAR_SCALE}>
      {highlighted ? (
        <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.03, 0]} receiveShadow>
          ...
          <meshBasicMaterial color={project.color} transparent opacity={0.38} />
        </mesh>
      ) : null}

      <RoundedBox args={[0.26, 0.12, 0.24]} radius={0.06} smoothness={4} position={[0, 0.76, 0.03]} castShadow>
        <meshStandardMaterial color={project.color} roughness={0.72} />
      </RoundedBox>

      <mesh position={[0, 1.23, 0.02]} rotation={[-0.06, 0, 0]} castShadow>
        <sphereGeometry args={[0.18, 18, 18]} />
        <meshStandardMaterial color="#f3c3a7" roughness={0.75} />
      </mesh>
      <mesh position={[0, 0.98, 0.03]} rotation={[0.26, 0, 0]} castShadow>
        <cylinderGeometry args={[0.18, 0.22, 0.46, 18]} />
        <meshStandardMaterial color={project.color} roughness={0.68} />
      </mesh>
      ...
      <RoundedBox
        ...
        radius={0.03}
        smoothness={4}
        position={[-0.1, 0.52, -0.11]}
        rotation={[Math.PI / 2.66, 0, 0]}
        castShadow
      >
...
function DeskPod({ slot, project, theme, highlighted, hovered, setHovered, onOpen }) {
...
function DeskPodContainer({ slot, project, theme, highlighted, onOpen, onHover }) {
...
function CameraRig() {
  ...
  const orbitLimits = React.useMemo(() => getOrbitLimits(CAMERA_PRESET), []);
...
function GalleryScene({ projects, theme, activeId, onOpen, onHover }) {
...
function ProjectGallery3D({ projects = [], theme = "dark", activeId, onOpen, onHover }) {
  return (
    <div className="clawlink-office-canvas">
      <Canvas
        ...
        dpr={[1, 1.25]}
        gl={{
          toneMappingExposure: theme === "light" ? 1 : 1.24,
