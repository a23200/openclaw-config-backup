...
const api = {
  ...
  materialLibraries: () => fetch('/api/material-libraries', noCache).then(r => r.json()),
  materialManifest: (name) => fetch(`/api/material-libraries/${encodeURIComponent(name)}/manifest`, noCache).then(r => r.json()),
  openMaterialLibrary: (library) => {
    const fd = new FormData();
    fd.set('library', library || '');
    return fetch('/api/material-libraries/open', { method: 'POST', body: fd }).then(r => r.ok ? r.json() : Promise.reject(r));
  },
  generateMaterial: (fd) => fetch('/api/generate-material', { method: 'POST', body: fd }).then(r => r.ok ? r.json() : Promise.reject(r)),
};
...
const STATE_LABELS = {
  ...
  video_ready: '视频就绪',
  post_processing: '后处理',
  narration_script_ready: '旁白脚本',
  tts_generating: 'TTS 生成',
  tts_ready: 'TTS 就绪',
  bgm_mixing: 'BGM 混音',
  subtitle_transcribing: 'Whisper 转写',
  subtitle_refining: '字幕清洗',
  subtitle_burning: '字幕烧录',
  final_packaging: '最终封装',
  mastered: '成片',
  publishing: '发布中',
  published: '已发布',
  failed: '失败',
  // material-library
  material_selecting: 'AI 选片',
  material_stitching: '拼接中',
  material_frames2video_submitted: 'frames2video 已投递',
  material_frames2video_queued: 'frames2video 排队',
  material_frames2video_ready: 'frames2video 就绪',
};
...
const MATERIAL_PRESETS = {
  auto: { tone: '', voice: 'calm_female', style: 'documentary', post: 'auto' },
  science: { tone: '清晰、可信、循序渐进，用一句现象引入，再给出直观解释', voice: 'steady_male', style: 'documentary', post: 'narrated' },
  explain: { tone: '口语化、节奏稳定、信息密度适中，像认真讲给朋友听', voice: 'calm_female', style: 'explain', post: 'narrated' },
  opinion: { tone: '有判断力但不激烈，先抛观点，再给理由，最后收束成一句态度', voice: 'clear_male', style: 'opinion', post: 'narrated' },
  list: { tone: '轻快、明确、有序，用几个关键词推进，每段画面对应一个角度', voice: 'bright_female', style: 'explain', post: 'narrated' },
  healing: { tone: '温柔、慢节奏、有画面感，少解释，多感受', voice: 'warm_female', style: 'emotional', post: 'narrated' },
  aesthetic: { tone: '克制、高级、留白，字幕只做点题，不解释画面', voice: 'calm_female', style: 'emotional', post: 'title_card' },
};

let TIMELINE = [];
let TIMELINES = { jimeng: [], material: [] };
let cachedJobs = [];
...
const fmtMixParams = (params) => {
  if (!params || typeof params !== 'object') return '—';
  const parts = [];
  if (params.ducking) parts.push(String(params.ducking).replace('_or_fallback', ''));
  if (Number.isFinite(params.bgm_volume)) parts.push(`BGM ${Math.round(params.bgm_volume * 100)}%`);
  if (Number.isFinite(params.voice_gain)) parts.push(`人声 ${params.voice_gain.toFixed(2)}x`);
  return parts.join(' · ') || '—';
};
...
function onTaskTypeChange(value) {
  const jimeng = $('#fs-jimeng');
  const material = $('#fs-material');
  if (jimeng) {
    jimeng.hidden = (value !== 'jimeng');
    jimeng.disabled = (value !== 'jimeng');
  }
  if (material) {
    material.hidden = (value !== 'material');
    material.disabled = (value !== 'material');
  }
  // toggle required on jimeng-only text fields (so material submissions pass)
  const jimengRequired = $$('#fs-jimeng [name="first_prompt"], #fs-jimeng [name="last_prompt"], #fs-jimeng [name="video_prompt"]');
  jimengRequired.forEach(el => { el.required = (value === 'jimeng'); });
}

function applyMaterialPreset(category) {
  const preset = MATERIAL_PRESETS[category] || MATERIAL_PRESETS.auto;
  const tone = $('#mat-tone');
  const voice = $('#mat-voice');
  const style = $('#mat-style');
  if (tone && !tone.dataset.userEdited) tone.value = preset.tone || '';
  if (voice) voice.value = preset.voice || 'calm_female';
  if (style) style.value = preset.style || 'documentary';
  if (preset.post) selectStrategy('post_mode', preset.post);
}
...
function bindForm() {
  $('#job-form').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (!fd.get('enable_natural_voiceover')) fd.set('enable_natural_voiceover', '');
      const taskType = fd.get('task_type') || 'jimeng';
      if (taskType === 'material') {
        const clipsJson = $('#mat-clips-json').value || '';
        if (!clipsJson) {
          throw new Error('请先点"AI 选片"挑出片段');
        }
        fd.set('clips', clipsJson);
        const materialStyle = $('#mat-style')?.value || MATERIAL_PRESETS[$('#mat-category')?.value || 'auto']?.style || 'documentary';
        fd.set('narration_style', materialStyle);
        // strip jimeng-only blank fields (server also tolerates empties)
        fd.delete('first_prompt');
        fd.delete('last_prompt');
        fd.delete('video_prompt');
      } else {
        // Keep material-category controls out of plain Jimeng tasks.
        [
          'library', 'clips', 'stitch_mode', 'topic', 'target_duration_sec',
          'material_category', 'narration_tone', 'voice_profile',
          'mat_narration_style', 'subtitle_style',
        ].forEach(name => fd.delete(name));
      }
      ...
      $$('.seg, .strategy-grid').forEach(group => {
        ...
        const field = group.dataset.field;
        const hidden = document.querySelector(`input[name="${field}"]`);
        if (hidden) hidden.value = items[0]?.dataset.value || '';
      });
      $('#audio-ref-field').hidden = true;
      $('#title-card-field').hidden = true;
      onTaskTypeChange('jimeng');
      $('#mat-clips-json').value = '';
      $('#clip-pick-list').innerHTML = '';
      const narration = $('#mat-narration');
      if (narration) narration.value = '';
      const tone = $('#mat-tone');
      if (tone) tone.dataset.userEdited = '';
      toast('任务已提交，开始 Claw Link 之旅 ✨', 'ok');
      loadJobs();
    } catch (err) {
...
// ---------- material library ----------
async function loadMaterialLibraries() {
  const sel = $('#mat-library');
  if (!sel) return;
  try {
    const libs = await api.materialLibraries();
    if (!libs.length) {
      sel.innerHTML = `<option value="">未检测到素材库（~/Movies/素材/）</option>`;
      return;
    }
    sel.innerHTML = libs.map(l =>
      `<option value="${escapeHtml(l.name)}">${escapeHtml(l.name)} · ${l.clip_count} 段 · ${fmtDuration(l.total_duration_sec || 0)}</option>`
    ).join('');
  } catch (err) {
...
async function onAiPick() {
  const library = $('#mat-library').value || '';
  const topic = $('#mat-topic').value.trim();
  const stitch = document.querySelector('input[name="stitch_mode"]').value || 'concat';
  const target = $('#target_duration_sec').value || '15';
  const category = $('#mat-category')?.value || 'auto';
  const tone = $('#mat-tone')?.value || '';
  const voice = $('#mat-voice')?.value || 'calm_female';
  const hint = $('#mat-hint');
  const btn = $('#btn-ai-pick');
  if (!library) { toast('先选一个素材库', 'err'); return; }
  if (!topic) { toast('主题不能为空', 'err'); return; }
  btn.disabled = true; btn.classList.add('loading');
  hint.textContent = 'GPT · 5.4 正在从清单里挑片段…';
  try {
    const fd = new FormData();
    fd.set('library', library);
    fd.set('topic', topic);
    fd.set('stitch_mode', stitch);
    fd.set('target_duration_sec', target);
    fd.set('material_category', category);
    fd.set('narration_tone', tone);
    fd.set('voice_profile', voice);
    const data = await api.generateMaterial(fd);
    renderClipPicks(data.clips || []);
    $('#mat-clips-json').value = JSON.stringify(data.clips || []);
    // populate publish fields if user hasn't filled them
    const pt = document.querySelector('[name="publish_title"]');
    const pd = document.querySelector('[name="publish_description"]');
    const tt = document.querySelector('[name="title"]');
    if (tt && !tt.value) tt.value = data.suggested_title || topic;
    if (pt && !pt.value) pt.value = data.suggested_title || '';
    if (pd && !pd.value) pd.value = data.suggested_description || '';
    if (data.title_card_text) {
      const tc = document.querySelector('[name="title_card_text"]');
      if (tc) tc.value = data.title_card_text;
    }
    if (data.narration_text) {
      const narration = $('#mat-narration');
      if (narration) narration.value = data.narration_text;
    }
    if (data.narration_tone) {
      const toneInput = $('#mat-tone');
      if (toneInput) toneInput.value = data.narration_tone;
    }
    if (data.voice_profile && $('#mat-voice')) $('#mat-voice').value = data.voice_profile;
    if (data.narration_style && $('#mat-style')) $('#mat-style').value = data.narration_style;
    if (data.suggested_post_mode) selectStrategy('post_mode', data.suggested_post_mode);
    const tot = (data.clips || []).reduce((s, c) => s + (c.duration_sec || 0), 0);
    const engine = data.engine === 'fallback' ? '顺序兜底' : `模型 ${data.engine}`;
    const modeText = POST_MODE_LABELS[data.suggested_post_mode] || data.suggested_post_mode || '自动';
    hint.textContent = `✓ ${(data.clips || []).length} 段 · 约 ${fmtDuration(tot)} · ${modeText} · ${engine}`;
    toast(`已挑出 ${(data.clips || []).length} 段`, 'ok');
  } catch (err) {
    toast('选片失败', 'err');
    hint.textContent = '选片失败，请稍后重试';
  } finally {
    btn.disabled = false; btn.classList.remove('loading');
  }
}

async function openSelectedMaterialLibrary() {
  const sel = $('#mat-library');
  const library = sel?.value || '';
  try {
    const res = await api.openMaterialLibrary(library);
    toast(`已打开：${res.path}`, 'ok');
  } catch {
    toast('打开本地素材库失败', 'err');
  }
}

function renderClipPicks(clips) {
  const ol = $('#clip-pick-list');
  if (!clips.length) { ol.innerHTML = ''; return; }
  ol.innerHTML = clips.map((c, i) => `
    <li class="clip-pick-item" title="${escapeHtml(c.reason || '')}">
      <span class="clip-order">${String(i + 1).padStart(2, '0')}</span>
      <span class="clip-name">${escapeHtml(c.file)}</span>
      <span class="clip-dur">${fmtDuration(c.duration_sec || 0)}</span>
      ${c.reason ? `<span class="clip-reason">${escapeHtml(c.reason)}</span>` : ''}
    </li>
  `).join('');
}
...
function bindMaterial() {
  const btn = $('#btn-ai-pick');
  if (btn) btn.addEventListener('click', onAiPick);
  const openBtn = $('#btn-open-material-library');
  if (openBtn) openBtn.addEventListener('click', openSelectedMaterialLibrary);
  const category = $('#mat-category');
  if (category) category.addEventListener('change', (e) => applyMaterialPreset(e.target.value));
  const tone = $('#mat-tone');
  if (tone) tone.addEventListener('input', () => { tone.dataset.userEdited = tone.value ? '1' : ''; });
}

function timelineFor(job) {
  const t = job?.task_type === 'material' ? TIMELINES.material : TIMELINES.jimeng;
  return (t && t.length) ? t : TIMELINE;
}
...
function renderSidebar(jobs) {
  ...
  list.innerHTML = filtered.map(j => {
    ...
    const tl = timelineFor(j);
    const total = tl.length || 13;
    let idx = tl.indexOf(j.status);
...
async function refreshDetail() {
  ...
  $('#d-tts-engine').textContent = job.post?.tts_engine || '—';
  $('#d-subtitle-mode').textContent = job.subtitle_mode || '—';
  const isMaterial = job.task_type === 'material';
  ['#d-row-narration-style', '#d-row-voice-profile', '#d-row-subtitle-style', '#d-row-mix-params']
    .forEach(selector => {
      const row = $(selector);
      if (row) row.hidden = !isMaterial;
    });
  const narrationStyle = isMaterial ? (job.post?.narration_style || job.input?.narration_style || '') : '';
  const voiceProfile = isMaterial ? (job.post?.voice_profile || job.input?.voice_profile || job.post?.voice_style_config?.voice_profile || '') : '';
  const subtitleStyle = isMaterial ? (job.post?.subtitle_style || '') : '';
  $('#d-narration-style').textContent = NARRATION_STYLE_LABELS[narrationStyle] || narrationStyle || '—';
  $('#d-voice-profile').textContent = VOICE_PROFILE_LABELS[voiceProfile] || voiceProfile || '—';
  $('#d-subtitle-style').textContent = SUBTITLE_STYLE_LABELS[subtitleStyle] || subtitleStyle || '—';
  $('#d-mix-params').textContent = fmtMixParams(job.post?.mix_params);
  ...
  $('#d-narration').textContent = job.post?.narration_text || '—';
  $('#d-bgm').textContent = job.post?.bgm_path || job.input?.bgm_path || '—';

  // previews & files
  renderPreviews(job);
  renderKeyFiles(job);
  renderAudioWorkbench(job);
  await renderSubtitlePreview(job);
  renderFiles(job);
...
function renderPublishSection(job) {
  const pf = preflightCache || { ready: false, port_open: false, cdp_ok: false, unchecked: true };
  const dot = $('#d-pf-dot');
  const txt = $('#d-pf-text');
  if (pf.unchecked) {
    dot.className = 'pf-dot';
    txt.textContent = '发布环境未检测 · 点击按钮后才检查';
  } else if (pf.ready) {
  ...
  if (pf.unchecked) {
    brDot.className = 'pf-dot';
    brTxt.textContent = '不会自动检测发布环境；点击后先检查，不可用时再启动调试 Chrome';
    brBtn.hidden = false;
    brBtn.textContent = '🔎 检查发布环境';
    brBtn.dataset.mode = 'check';
  } else if (pf.ready) {
...
function renderTopPreflight() {
  ...
  if (!pf) {
    dot.className = 'pf-dot';
    txt.textContent = '发布环境未检测 · 点击右侧按钮检查';
    if (btn) {
      btn.hidden = false;
      btn.textContent = '检查发布环境';
...
async function testBrowserConnection(btn) {
  const mode = ['check', 'launch', 'test'].includes(btn?.dataset.mode) ? btn.dataset.mode : 'check';
  const original = btn ? btn.textContent : '';
  if (btn) {
    btn.disabled = true;
    btn.textContent = mode === 'launch' ? '启动中…' : (mode === 'test' ? '测试中…' : '检查中…');
  }
  try {
    if (mode === 'check') {
      await refreshPreflight(false);
      if (preflightCache?.ready) {
        ...
        if (res.ok) {
          toast(res.already_running ? '✓ 调试 Chrome 已在运行' : '✓ 调试 Chrome 已启动 · 如弹窗询问远程调试，请点允许', 'ok');
          await refreshPreflight(false);
        } else {
    ...
    if (btn) {
      btn.disabled = false;
      if (mode !== 'check') btn.textContent = original;
    }
    if (mode !== 'check') await refreshPreflight(false);
  }
}

function renderTimeline(job) {
  const ol = $('#d-timeline');
  const tl = timelineFor(job);
  if (job.status === 'failed') {
    ol.innerHTML = tl.map((s, i) => {
      return `<li class="done"><span class="tn">${i + 1}</span>${STATE_LABELS[s] || s}</li>`;
    }).join('') + `<li class="failed"><span class="tn">!</span>失败</li>`;
    return;
  }
  const curIdx = tl.indexOf(job.status);
  ol.innerHTML = tl.map((s, i) => {
    let cls = '';
...
const VIDEO_INFLIGHT_STATES = new Set([
  ...
  'material_frames2video_submitted', 'material_frames2video_queued',
]);
const MATERIAL_STITCH_STATES = new Set(['material_stitching']);
...
function renderPreviews(job) {
  ...
  // material concat/stitch in progress — show stitching placeholder
  if (!videoFound && isMaterial && MATERIAL_STITCH_STATES.has(job.status)) {
    items.push(`
      <div class="preview-item queue-card">
        <div class="ph queue-ph">
          <div class="queue-status">${STATE_LABELS[job.status] || job.status}</div>
          <div class="queue-rank">ffmpeg</div>
          <div class="queue-bar"><span style="width:40%"></span></div>
          <div class="queue-sub">拼接本地片段…</div>
        </div>
        <div class="cap"><span>素材 · 拼接中</span></div>
      </div>`);
    videoFound = true;
  }
  ...
  host.innerHTML = items.length ? items.join('') : `<div class="preview-empty">还没有产物，稍等 Claw Link 出片…</div>`;
...
function renderKeyFiles(job) {
  const host = $('#d-key-files');
  const artifacts = job.artifacts || [];
  const wanted = ['narration.mp3', 'narration_normalized.mp3', 'mixed_audio.mp3', 'raw_subtitles.srt', 'subtitles.srt', 'subtitles.ass', 'final_with_audio.mp4', 'publish_preview.json', 'publish_checklist.json'];
  const hits = artifacts.filter(a => wanted.includes(a.name));
  if (!hits.length) {
    host.innerHTML = '<div class="empty-inline">暂无关键中间产物</div>';
    return;
  }
  host.innerHTML = hits.map(a => `
    <a class="file-row" href="/api/jobs/${job.id}/files/${encodeURIComponent(a.name)}" target="_blank" rel="noopener">
      <span class="file-name">${escapeHtml(a.name)}</span>
      <span class="file-meta">${fmtSize(a.size)}</span>
    </a>
  `).join('');
}

function renderAudioWorkbench(job) {
  const host = $('#d-audio-workbench');
  if (!host) return;
  const artifacts = job.artifacts || [];
  const audioNames = ['narration.mp3', 'narration_normalized.mp3', 'mixed_audio.mp3'];
  const hits = artifacts.filter(a => audioNames.includes(a.name));
  if (!hits.length) {
    host.innerHTML = '<div class="empty-inline">暂无可试听音频</div>';
    return;
  }
  host.innerHTML = hits.map(a => {
...
async function renderSubtitlePreview(job) {
  const host = $('#d-subtitle-preview');
  if (!host) return;
  const artifacts = job.artifacts || [];
  const clean = artifacts.find(a => a.name === 'subtitles.srt');
  const raw = artifacts.find(a => a.name === 'raw_subtitles.srt');
  const target = clean || raw;
  if (!target) {
    host.textContent = '暂无字幕预览';
    return;
  }
  try {
    const url = `/api/jobs/${job.id}/files/${encodeURIComponent(target.name)}?_=${Date.now()}`;
    const text = await fetch(url, noCache).then(r => r.ok ? r.text() : '');
    host.textContent = `${target.name}\n\n${text.slice(0, 3000) || '空文件'}`;
  } catch {
    host.textContent = '字幕预览读取失败';
  }
}
...
(async function init() {
  try {
    ...
    TIMELINES = meta.timelines || { jimeng: TIMELINE, material: [] };
  } catch {
    TIMELINE = [
    ...
    TIMELINES = { jimeng: TIMELINE, material: [] };
  }
  bindSegments();
  onTaskTypeChange(document.querySelector('input[name="task_type"]')?.value || 'jimeng');
  bindForm();
  bindDrawerActions();
  bindFilters();
  bindTabs();
