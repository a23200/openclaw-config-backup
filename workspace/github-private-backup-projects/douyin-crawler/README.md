...
如果 `8000` 端口已被别的项目占用，可以改端口启动：

```bash
LEAD_OPS_PORT=8001 zsh /Users/mac/Desktop/抖音爬虫/scripts/start_server.sh
```
...
- 任务工作台的批量执行模式仅负责打开私信并填入文案，任务会停在 `prepared` 等待人工确认发送
- 线索列表里的“代我发送‘您好’”会尝试自动发送；若平台未确认发送成功，页面会明确提示你手动点发送
