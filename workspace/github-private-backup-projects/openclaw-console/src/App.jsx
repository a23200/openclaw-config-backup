...
const WorkstationCockpit = React.lazy(() => import("./components/WorkstationCockpit.jsx"));
...
class SceneErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info) {
    console.warn("3D scene crashed", error, info);
  }
  render() {
    if (this.state.error) {
      return (
...
const PROJECTS = [
  ...
  {
    id: "website-engineer",
    name: "website-clone-security",
    role: "网站工程师",
    emoji: "🌐",
    color: "#3b82f6",
    techStack: "网站复刻 · 漏洞巡检 · Browser Automation",
    description: "在工位面板里直接复刻网站并打开演示页，同时执行被动漏洞巡检。",
  },
  {
    id: "jimeng-ui",
    name: "jimeng-ui",
  ...
  },
  {
    ...
    techStack: "FastAPI · Vue3 · 自动驾驶生成",
    description: "PlotPilot 墨枢 · 长篇创作平台 · Story Bible · 知识图谱 · 风格分析。",
    launch: {
  ...
  },
  {
    ...
    color: "#14b8a6",
    url: "",
    techStack: "链上空投 · 自动化操作",
    description: "后期填充 · 空投任务执行与撸羊毛流水线。",
  },
];
...
export default function App() {
  const [activeView, setActiveView] = React.useState("browser");
  ...
  const [isDockHidden, setIsDockHidden] = React.useState(() => {
    if (typeof window === "undefined") return true;
    try {
      const stored = window.localStorage.getItem("clawlink-dock-hidden");
  ...
  const [focusedProject, setFocusedProject] = React.useState({ id: null, nonce: 0 });
  const [cockpitProjectId, setCockpitProjectId] = React.useState(null);
  const [projectStatusMap, setProjectStatusMap] = React.useState({});
  const [launchingProjectId, setLaunchingProjectId] = React.useState(null);
  ...
  const launchAllTimerRef = React.useRef(null);
  const launchOneTimerRef = React.useRef(null);
  React.useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      window.localStorage.setItem("clawlink-dock-hidden", isDockHidden ? "1" : "0");
    } catch {}
  }, [isDockHidden]);

  React.useEffect(() => {
    return () => {
      if (launchAllTimerRef.current) clearTimeout(launchAllTimerRef.current);
      if (launchOneTimerRef.current) clearTimeout(launchOneTimerRef.current);
    };
  }, []);
  ...
  React.useEffect(() => {
    ...
    const timer = setInterval(refreshProjectStatus, 15000);
    return () => clearInterval(timer);
  }, [refreshProjectStatus]);

  const handleOpenProject = React.useCallback((project) => {
    if (!project?.id) return;
    if (project.id === "website-engineer") {
      setCockpitProjectId(project.id);
      return;
    }
  ...
  const handleFocusProject = React.useCallback((project) => {
    if (!project?.id) return;
    setActiveProjectId(project.id);
    setFocusedProject((current) => ({ id: project.id, nonce: current.nonce + 1 }));
    // 目前只有网站工程师工位有专门的 cockpit 对话,其他工位聚焦时不弹
    if (project.id === "website-engineer") {
      setCockpitProjectId(project.id);
    }
  }, []);

  const cockpitProject = React.useMemo(
    () => PROJECTS.find((p) => p.id === cockpitProjectId) || null,
    [cockpitProjectId],
  );

  const [launchAllActive, setLaunchAllActive] = React.useState(false);
  const handleLaunchAllProjects = React.useCallback(async () => {
    const candidates = PROJECTS.filter((project) => {
      if (!project.launch?.command) return false;
      const status = projectStatusMap[project.id];
    ...
    setLaunchAllActive(true);
    const pendingIds = new Set();
    const startResults = await Promise.allSettled(
      candidates.map((project) =>
        fetch(`/api/projects/${encodeURIComponent(project.id)}/start`, { method: "POST" }).then(
    ...
    startResults.forEach((result, index) => {
      ...
      if (result.status === "rejected") {
        console.warn("launch all failed", project.id, result.reason);
        return;
      }
      ...
      if (response.ok || response.status === 409) {
        ...
        console.warn("launch all non-ok", project.id, response.status);
      }
    });

    if (!pendingIds.size) {
      setLaunchAllActive(false);
      return;
    }
    ...
    const scheduleNext = () => {
      launchAllTimerRef.current = setTimeout(() => {
        poll().catch((err) => {
          console.warn("launch-all poll crashed, stopping", err);
          if (isMountedRef.current) setLaunchAllActive(false);
        });
      }, 2500);
    };
    const poll = async () => {
      ...
      try {
        try {
          ...
          if (response.ok) {
            ...
            for (const item of payload?.projects || []) {
              next[item.id] = item;
              if (item.reachable) pendingIds.delete(item.id);
            }
            if (isMountedRef.current) setProjectStatusMap(next);
          }
        } catch (error) {
          console.warn("launch-all poll fetch failed", error);
        }
        if (!pendingIds.size || Date.now() - startedAt >= maxWaitMs) {
          if (isMountedRef.current) setLaunchAllActive(false);
          return;
        }
        scheduleNext();
      } catch (error) {
        console.warn("launch-all poll crashed, stopping", error);
        if (isMountedRef.current) setLaunchAllActive(false);
      }
    };
    scheduleNext();
  }, [projectStatusMap, refreshProjectStatus]);

  const handleLaunchProject = React.useCallback(
    async (project) => {
      if (!project?.id || !project?.launch) return;
      if (isMountedRef.current) setLaunchingProjectId(project.id);

      try {
        const response = await fetch(`/api/projects/${encodeURIComponent(project.id)}/start`, {
          method: "POST",
        });
        const payload = await response.json().catch(() => ({}));
        if (!response.ok && response.status !== 409) {
          console.warn("launch project failed", project.id, payload);
          if (isMountedRef.current) {
            setLaunchingProjectId((current) => (current === project.id ? null : current));
          }
          refreshProjectStatus();
          return;
        }
      } catch (error) {
        console.warn("launch project failed", project.id, error);
        if (isMountedRef.current) {
          setLaunchingProjectId((current) => (current === project.id ? null : current));
        }
        refreshProjectStatus();
      ...
      const pollOnce = async () => {
        ...
        try {
          ...
          if (response.ok) {
            const payload = await response.json();
            const next = {};
            for (const item of payload?.projects || []) next[item.id] = item;
            if (isMountedRef.current) setProjectStatusMap(next);
            if (next[project.id]?.reachable) {
              if (isMountedRef.current) {
                setLaunchingProjectId((current) => (current === project.id ? null : current));
              }
              return true;
      ...
      const schedule = () => {
        launchOneTimerRef.current = setTimeout(async () => {
          try {
            ...
            console.warn("launch project poll crashed", project.id, error);
            if (isMountedRef.current) {
              setLaunchingProjectId((current) => (current === project.id ? null : current));
            }
          }
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      console.warn("无法写入 localStorage（隐私模式或配额已满）", error);
    }
  }, [config]);
  ...
  React.useEffect(() => {
    ...
    return () => {
      ...
      source.removeEventListener("message", handleMessageEvent);
      source.removeEventListener("history", handleHistoryEvent);
  ...
  function renderSceneView() {
    ...
    const onlineProjectCount = PROJECTS.reduce((count, project) => {
      if (project.id === "clawlink") return count + 1;
      return count + (projectStatusMap[project.id]?.reachable ? 1 : 0);
    }, 0);
    const launchableOfflineCount = PROJECTS.reduce((count, project) => {
      if (!project.launch?.command) return count;
      if (projectStatusMap[project.id]?.reachable) return count;
      return count + 1;
    }, 0);
    ...
    return (
      <div className="scene-workspace">
        <React.Suspense fallback={null}>
          <WorkstationCockpit
            open={Boolean(cockpitProject)}
            project={cockpitProject}
            onClose={() => setCockpitProjectId(null)}
            mode={mode}
            sessionKey={sessionKey}
            agentId={agentId}
            sessionEndpoint={sessionEndpoint}
            chatStreamEndpoint={buildChatStreamEndpoint(endpoint)}
            onSessionKeyResolved={(nextKey) => {
              if (nextKey && nextKey !== sessionKey) {
                updateConfig("sessionKey", nextKey);
              }
            }}
          />
        </React.Suspense>
        ...
        <section className="scene-stage-panel app-panel">
          <div className="scene-canvas-shell">
            <SceneErrorBoundary>
              <React.Suspense fallback={<SceneLoadingFallback />}>
                <ProjectGallery3D
                  projects={PROJECTS}
                  theme={theme}
                  activeId={activeProjectId}
                  focusId={focusedProject.id}
                  focusToken={focusedProject.nonce}
                  onOpen={handleOpenProject}
                  onHover={setActiveProjectId}
                  statusMap={projectStatusMap}
                  launchingId={launchingProjectId}
          ...
          <div className="scene-stage-header">
            <div className="scene-stage-overview">
              <div className="scene-stage-copy">
                <div className="topbar-eyebrow">ClawLink · AI 一人公司</div>
                <h2 className="scene-stage-title text-[var(--text-primary)]">我和我的 {PROJECTS.length} 位 AI 同事</h2>
                <div className="scene-stage-meta text-xs text-[var(--text-secondary)]">
                  <span>一人撑起一家 AI 公司</span>
                  <span>·</span>
                  <span>{onlineProjectCount} 在线 / {launchableOfflineCount} 待启动</span>
                  <span>·</span>
              ...
              <button
                type="button"
                onClick={handleLaunchAllProjects}
                disabled={launchAllActive || launchableOfflineCount === 0}
                className="scene-launch-all-btn"
                style={{
                  background:
                    launchAllActive || launchableOfflineCount === 0
                  ...
                  cursor:
                    launchAllActive || launchableOfflineCount === 0 ? "not-allowed" : "pointer",
                }}
                title={
                  launchableOfflineCount === 0
                    ? "全员已上班"
                    : `启动 ${launchableOfflineCount} 位离线同事`
                }
              >
                <span>☕</span>
                <span>
            ...
            <div className="scene-preset-row">
              {PROJECTS.map((project) => {
                ...
                return (
                  <button
                    key={project.id}
                    type="button"
                    className={`scene-preset-chip ${highlight ? "active" : ""} ${isFocused ? "focused" : ""}`}
                    style={
                      highlight
                    ...
                    title={`聚焦到 ${project.role} · 打开工位对话`}
                  >
                    {isFocused ? <span className="scene-preset-chip-dot" style={{ background: project.color }} /> : null}
              ...
              {focusedProject.id ? (
                <button
                  type="button"
                  className="scene-preset-chip reset"
                  onClick={() => {
                    setActiveProjectId(null);
                    setFocusedProject({ id: null, nonce: 0 });
                  }}
                  title="取消聚焦,相机回到全景视角"
                >
        ...
        {!isDockHidden ? (
          <aside className="app-dock hidden xl:flex">
            ...
            <div className="dock-footer">
              ...
              <div className="mt-4 text-xs uppercase tracking-[0.18em] text-[var(--text-muted)]">{BRANDING.gatewayName}</div>
              ...
              <div className="topbar-right-cluster">
                ...
                <div className="topbar-tools">
                  <button
                    ...
                    onClick={() => setIsDockHidden((current) => !current)}
                    aria-label={isDockHidden ? "显示左侧标签栏" : "隐藏左侧标签栏"}
                    title={isDockHidden ? "显示左侧标签栏" : "隐藏左侧标签栏"}
                  >
                    {isDockHidden ? <ChevronRight className="h-4 w-4" /> : <X className="h-4 w-4" />}
                  </button>
                  <button
                    type="button"
                    className="icon-shell"
