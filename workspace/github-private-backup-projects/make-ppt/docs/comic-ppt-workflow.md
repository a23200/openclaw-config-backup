...
当网站里选择 `漫画风 PPT` 或 `漫画风 PPT（GPT）` 模板时，不走普通商务模板链路，而是走「参考图 + 真生图 + 图文融合」专用流程。
...
1. 必须把参考图喂给当前选中的生图模型；默认支持 `Gemini` 与 `GPT Image / OpenAI-compatible` 两条路线。
...
6. 漫画模板网站链路默认启用 `v7` 融合规则：主风格参考 + 布局参考 + 按页参考图。
...
- 按页融合参考：优先使用 `runtime/manual-watch/20260422-旧神陨落新神开眼-融合参考-v7-sanitized-refs/` 下的 `P1` ~ `P5` 去字样张
...
4. 每页调用当前选中的生图路线（Gemini 或 GPT Image），并把精简后的中文标题/要点直接交给模型出字。
5. 保存：
   - 原始 Gemini 底图
   - 最终页图
   - `slide-plan.json`
