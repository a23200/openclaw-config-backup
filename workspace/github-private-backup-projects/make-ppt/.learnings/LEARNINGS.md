...
**Logged**: 2026-04-22T04:58:29.838933+08:00
**Priority**: high
**Status**: pending
**Area**: backend

### Summary
漫画风 PPT 生成中，图内强制文案必须与场景提示词分离，且章节提炼应优先结论句而非子标题。

### Details
本次用户反馈显示，虽然图片风格与版式正常，但分镜中的文档分析结果被抽成了目录式子标题，且场景提示词包含过多文本语义，导致 Gemini 把额外文字扩写进图中。修复方式是：1）顶层章节优先提取加粗结论句与关键论断；2）只有顶层内容不足时才下钻子章节；3）场景提示只保留抽象视觉语义，不再复用分析句。

### Suggested Action
继续沿用“focus points for text card + abstract scene prompt for image”双轨模式，并在后续模板里默认禁止全图额外标签。

### Metadata
- Source: user_feedback
- Related Files: scripts/build_comic_manga_ppt.py
- Tags: comic_ppt, gemini, prompt_design, extraction

---
## [LRN-20260422-002] best_practice

**Logged**: 2026-04-22T13:35:00+08:00
**Priority**: high
**Status**: pending
**Area**: backend

### Summary
漫画风 PPT 的“融合版”应保留 v5 的精确文案约束，同时继续喂布局/旧页参考图，避免只剩干净文字块却丢掉优秀场景元素。

### Details
本次用户明确反馈：当前 v5 的逐页文案切分已明显改善，但更早一版的融合效果更强，图片里的业务元素、卡片结构和图内文字与场景的融合感更好。根因是后续 prompt 为了控字，把参考链路简化成只喂风格图并强调“单一编辑块”，导致画面更稳但元素丰富度下降。更优做法是继续保留“exact lines + 无额外文字”的强约束，同时恢复第二张布局参考和同页旧图参考，让模型继承已有元素组织与文字嵌入方式。

### Suggested Action
为 `scripts/build_comic_manga_ppt.py` 保留按页融合参考入口；后续用户要求“融合版”时，默认使用风格参考图 + 布局参考图 + 同页旧图参考，并在 README/RESULT 中明确记录参考来源。

### Metadata
- Source: user_feedback
- Related Files: scripts/build_comic_manga_ppt.py
- Tags: comic_ppt, gemini, fusion_reference, prompt_design, memory
