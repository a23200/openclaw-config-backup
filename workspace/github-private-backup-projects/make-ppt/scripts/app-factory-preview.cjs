...
const comicDefaultReferenceMode = normalizeComicReferenceMode(
  process.env.APP_FACTORY_COMIC_REFERENCE_MODE || process.env.COMIC_REFERENCE_MODE || 'prompt_only',
);
const comicFusionReferenceDir = resolveExistingPath(
  [
    process.env.APP_FACTORY_COMIC_PAGE_REFERENCE_DIR,
    path.join(repoRoot, 'runtime', 'manual-watch', '20260422-旧神陨落新神开眼-修复提炼-正式生成', 'comic-assets'),
    path.join(repoRoot, 'runtime', 'manual-watch', '20260422-旧神陨落新神开眼-融合参考-v7-sanitized-refs'),
  ],
  process.env.APP_FACTORY_COMIC_PAGE_REFERENCE_DIR || '',
);
const comicGptImageApiKey = String(process.env.COMIC_GPT_IMAGE_API_KEY || process.env.COMIC_OPENAI_API_KEY || '').trim();
const comicGptImageBaseUrl = String(process.env.COMIC_GPT_IMAGE_BASE_URL || process.env.COMIC_OPENAI_BASE_URL || 'https://api.openai.com/v1').replace(/\/+$/, '');
const comicGptImageModel = String(process.env.COMIC_GPT_IMAGE_MODEL || process.env.COMIC_OPENAI_IMAGE_MODEL || 'gpt-image-2').trim();
const comicGptImageSize = String(process.env.COMIC_GPT_IMAGE_SIZE || process.env.COMIC_OPENAI_IMAGE_SIZE || 'auto').trim();
const comicGptRouteEnabled = Boolean(comicGptImageApiKey && comicGptImageBaseUrl && comicGptImageModel);

function normalizeComicReferenceMode(value) {
  const normalized = String(value || '').trim().toLowerCase().replace(/-/g, '_');
  if (['prompt', 'prompt_only', 'pure_prompt', 'no_reference', 'none'].includes(normalized)) return 'prompt_only';
  if (['style', 'style_only', 'style_reference'].includes(normalized)) return 'style_only';
  return 'full';
}

function comicTemplateSummary(referenceMode) {
  if (referenceMode === 'prompt_only') {
    return '纯提示词图文融合，按文档内容自动定页';
  }
  if (referenceMode === 'style_only') {
    return '风格参考图文融合，不套固定版式，按文档内容自动定页';
  }
  return '参考图图文融合，按文档内容自动定页';
}

function comicTemplateStyle(referenceMode) {
  if (referenceMode === 'prompt_only') {
    return '商业漫画 / 图文融合 / 纯提示词';
  }
  if (referenceMode === 'style_only') {
    return '商业漫画 / 图文融合 / 风格参考';
  }
  return '商业漫画 / 图文融合 / 参考图';
}

function buildComicTemplate({ name, displayName, engineLabel, imageProvider, referenceMode, color, sortOrder }) {
  return {
    name,
    displayName,
    category: 'comic',
    summary: comicTemplateSummary(referenceMode),
    color,
    style: comicTemplateStyle(referenceMode),
...
const sharedWorkflowTemplates = {
  ...(comicGptRouteEnabled
    ? {
        comic_manga_ppt_gpt_prompt: buildComicTemplate({
          name: 'Comic Manga PPT · GPT Image · Prompt',
          displayName: '提示词模式一',
          engineLabel: 'GPT Image 漫画生图',
          imageProvider: 'gpt',
          referenceMode: 'prompt_only',
          color: '蓝橙漫画叙事',
          sortOrder: 10,
        }),
        comic_manga_ppt_gpt_ref: buildComicTemplate({
          name: 'Comic Manga PPT · GPT Image · Ref',
          displayName: '参考图模式一',
          engineLabel: 'GPT Image 漫画生图',
          imageProvider: 'gpt',
          referenceMode: 'full',
    ...
    : {}),
  comic_manga_ppt_prompt: buildComicTemplate({
    name: 'Comic Manga PPT · Gemini · Prompt',
    displayName: '提示词模式二',
  ...
  comic_manga_ppt_ref: buildComicTemplate({
    name: 'Comic Manga PPT · Gemini · Ref',
    displayName: '参考图模式二',
    engineLabel: 'Gemini 漫画生图',
    imageProvider: 'gemini',
    referenceMode: 'full',
...
const providerCatalog = {
  ppt_master: {
    ...
    defaultTemplateKey: 'comic_manga_ppt_gpt_prompt',
  },
  plus_ai: {
    key: 'plus_ai',
    label: 'Plus AI',
...
const activeProviderKey = 'ppt_master';
...
function resolveTemplateRuntime(providerKey, templateKey) {
  ...
  if (workflow === 'comic_manga') {
    const imageProvider = templateMeta.imageProvider === 'gpt' ? 'gpt' : 'gemini';
    const referenceMode = normalizeComicReferenceMode(templateMeta.referenceMode || comicDefaultReferenceMode);
    return {
      providerKey: imageProvider === 'gpt' ? 'comic_gpt' : 'comic_gemini',
...
async function runStructuredProcess(command, args, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', async (code) => {
      ...
      const error = new Error(stderr.trim() || stdout.trim() || `${path.basename(command)} exited with code ${code}`);
      error.stdout = stdout;
      error.stderr = stderr;
...
function comicImageEnvForProvider(imageProvider) {
  if (imageProvider === 'gpt') {
    return {
      COMIC_IMAGE_PROVIDER: 'gpt',
      COMIC_OPENAI_API_KEY: comicGptImageApiKey,
      COMIC_OPENAI_BASE_URL: comicGptImageBaseUrl,
      COMIC_OPENAI_IMAGE_MODEL: comicGptImageModel,
      COMIC_OPENAI_IMAGE_SIZE: comicGptImageSize,
      COMIC_OPENAI_IMAGE_MAX_ATTEMPTS: process.env.COMIC_OPENAI_IMAGE_MAX_ATTEMPTS || '8',
      COMIC_OPENAI_IMAGE_RETRY_BASE_DELAY_SECONDS: process.env.COMIC_OPENAI_IMAGE_RETRY_BASE_DELAY_SECONDS || '3',
      COMIC_OPENAI_IMAGE_RETRY_MAX_DELAY_SECONDS: process.env.COMIC_OPENAI_IMAGE_RETRY_MAX_DELAY_SECONDS || '45',
      COMIC_OPENAI_IMAGE_RETRY_JITTER_SECONDS: process.env.COMIC_OPENAI_IMAGE_RETRY_JITTER_SECONDS || '1.5',
      COMIC_OPENAI_IMAGE_REQUEST_TIMEOUT_SECONDS: process.env.COMIC_OPENAI_IMAGE_REQUEST_TIMEOUT_SECONDS || '900',
      COMIC_OPENAI_IMAGE_LOW_SPEED_TIME_SECONDS: process.env.COMIC_OPENAI_IMAGE_LOW_SPEED_TIME_SECONDS || '240',
    };
  }
  return {
    COMIC_IMAGE_PROVIDER: 'gemini',
    GEMINI_IMAGE_MAX_ATTEMPTS: process.env.GEMINI_IMAGE_MAX_ATTEMPTS || '8',
    GEMINI_IMAGE_RETRY_BASE_DELAY_SECONDS: process.env.GEMINI_IMAGE_RETRY_BASE_DELAY_SECONDS || '3',
    GEMINI_IMAGE_RETRY_MAX_DELAY_SECONDS: process.env.GEMINI_IMAGE_RETRY_MAX_DELAY_SECONDS || '45',
    GEMINI_IMAGE_RETRY_JITTER_SECONDS: process.env.GEMINI_IMAGE_RETRY_JITTER_SECONDS || '1.5',
    GEMINI_IMAGE_REQUEST_TIMEOUT_SECONDS: process.env.GEMINI_IMAGE_REQUEST_TIMEOUT_SECONDS || '900',
    GEMINI_IMAGE_LOW_SPEED_TIME_SECONDS: process.env.GEMINI_IMAGE_LOW_SPEED_TIME_SECONDS || '240',
  };
}
...
async function createComicMangaProject(brief, projectSlug, reportProgress, reportLog) {
  ...
  const sourceMaterial = await prepareSourceMaterial(brief, reportProgress, reportLog);
  runs.push(...sourceMaterial.runs);
  const referenceMode = normalizeComicReferenceMode(brief.referenceMode || comicDefaultReferenceMode);
  ...
  const args = [
    ...
    '--native-text',
    '--image-provider',
    brief.imageProvider === 'gpt' ? 'gpt' : 'gemini',
    '--reference-mode',
    referenceMode,
  ];
  if (referenceMode === 'full' && comicFusionReferenceDir) {
    args.push('--page-reference-dir', comicFusionReferenceDir);
  }
  ...
  await callLog(
    ...
    'design',
    '漫画模板启动',
    `开始按文档内容分析页数，并进入${referenceMode === 'full' ? '参考图模式' : '提示词模式'}的漫画式设计生成流程。`,
  );
  ...
  try {
    processRun = await runStructuredProcess(repoPython, args, {
      cwd: repoRoot,
      env: {
        ...comicImageEnvForProvider(brief.imageProvider),
        COMIC_REFERENCE_MODE: referenceMode,
      },
    ...
    runs.push({
      ...
      stderr: error && typeof error === 'object' && 'stderr' in error ? String(error.stderr || '') : (error instanceof Error ? error.message : String(error)),
      error: error instanceof Error ? error.message : String(error),
    });
    return {
      provider: brief.provider,
...
async function runProjectPipeline(brief, projectSlug) {
  ...
  try {
    ...
    nextProject.exportPptxPath = resolvedArtifacts.exportPptxPath || pipeline.exportPptxPath || '';
    nextProject.exportSvgPptxPath = resolvedArtifacts.exportSvgPptxPath || pipeline.exportSvgPptxPath || '';
    ...
    JSON.stringify(
      {
        ...
        imageProvider: templateRuntime.imageProvider || '',
        referenceMode: templateRuntime.referenceMode || '',
      },
      null,
      2,
    ),
