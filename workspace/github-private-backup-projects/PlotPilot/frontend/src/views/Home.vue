      ...
      <div class="container">
        ...
        <header class="header">
          <div class="header-content">
            <h1 class="title">小说创作家 · 长篇叙事工作台</h1>
            <p class="subtitle">
          ...
          <n-space vertical :size="20">
            ...
            <n-space justify="end">
              <n-button
                ...
                round
                :loading="creating"
                :disabled="!newBook.premise.trim() || (!isArticleTier && (!newBook.genre || !newBook.worldPreset))"
                @click="handleCreate"
              >
                <template #icon>
                  <n-icon><IconSpark /></n-icon>
                </template>
                {{ isArticleTier ? '一键生成约 1 万字短文' : '建档并进入工作台' }}
              </n-button>
            </n-space>
          </n-space>
        </n-card>
        ...
        <footer class="home-footer">
          <span class="footer-brand">小说创作家</span>
        </footer>
      </div>
    </div>
...
const SIDEBAR_COLLAPSED_KEY = 'novel_creator_sidebar_collapsed'
...
/** V1 目标篇幅档（与高级自定义二选一） */
const lengthTier = ref<'article' | 'short' | 'standard' | 'epic'>('standard')
const isArticleTier = computed(() => lengthTier.value === 'article' && !showAdvanced.value)
const premisePlaceholder = computed(() =>
  isArticleTier.value
    ? '贴进你的题目或一句话梗概,一两行就够。\n\n例如:十年后在游戏厅再次遇见她。'
    : '用一段话写清主线与爽点预期（不超过 2000 字）…\n\n例如：废柴赘婿觉醒签到系统，从被退婚到一方巨擘。'
)
const lengthTierOptions = [
  {
    value: 'article' as const,
    title: 'D · 短文 / 单篇',
    hint: '约 1 万字（拆成 4 节 × 2500 字写完,总耗时约 2-3 分钟,跳过 Bible 与规划）',
  },
  {
...
const handleCreate = async () => {
  ...
  // 短文(article)模式只需要梗概,类型/世界观可空
  if (!isArticleTier.value) {
  ...
  try {
    ...
    if (isArticleTier.value) {
      // 短文模式:后端已自启托管,直接跳工作台看 SSE 流(跳过 Bible 设置向导)
      message.success('已开始生成 4 节短文,约 2-3 分钟完成')
