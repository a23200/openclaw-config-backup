  ...
  <div class="bible-panel">
    <header class="bible-hero">
      <div class="bible-hero-main">
        ...
        <p class="bible-lead">
          <strong>梗概锁定</strong>（主线与不可违背设定）与<strong>写作公约</strong>（人称、时态、叙事距离、基调）——全书的「写什么」与「怎么写」。
        </p>
...
const generatePremiseKnowledge = async () => {
  ...
  try {
    ...
    if ((res.premise_lock || '').trim()) {
      premiseLock.value = res.premise_lock.trim()
    } else {
      await loadPremiseLock()
    }
    message.success(
      res.message || `梗概已生成${typeof res.facts_count === 'number' ? `（${res.facts_count} 条知识）` : ''}`
    )
    window.dispatchEvent(new CustomEvent('aitext:knowledge:reload'))
  } catch (e: any) {
