...
def _is_article_novel(novel_id: str) -> bool:
    """一次轻量 SQL:novel 是否为短文模式(article tier)。查询失败按 False 处理,让管线正常跑。"""
    try:
        ...
        return bool(row) and (row.get("length_tier") or "") == "article"
    except Exception:
        return False
...
class ChapterAftermathPipeline:
    ...
    async def run_after_chapter_saved(
        ...
        短文模式(length_tier=article)直接返回,不跑叙事同步/文风/KG —— 短文不需要维持长期一致性。
        """
        out: Dict[str, Any] = {
            "drift_alert": False,
        ...
        if _is_article_novel(novel_id):
            logger.info("aftermath 跳过:短文模式 novel=%s ch=%s", novel_id, chapter_number)
            out["narrative_sync_ok"] = True
            out["skipped"] = "article_mode"
            return out
