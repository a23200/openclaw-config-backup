...
DEFAULT_AUTOPILOT_CONTEXT_MAX_TOKENS = 12000
DEFAULT_AUTOPILOT_TARGET_CHAPTER_WORDS = 2600
DEFAULT_AUTOPILOT_BEAT_MAX_TOKENS_RATIO = 1.25
...
class AutopilotDaemon:
    ...
    def __init__(
        ...
        self.autopilot_context_max_tokens = max(
            4000,
            int(
                os.getenv(
                    "AUTOPILOT_CONTEXT_MAX_TOKENS",
                    str(DEFAULT_AUTOPILOT_CONTEXT_MAX_TOKENS),
                )
            ),
        )
        self.autopilot_target_chapter_words = max(
            1200,
            int(
                os.getenv(
                    "AUTOPILOT_TARGET_CHAPTER_WORDS",
                    str(DEFAULT_AUTOPILOT_TARGET_CHAPTER_WORDS),
                )
            ),
        )
        self.autopilot_beat_max_tokens_ratio = max(
            1.0,
            float(
                os.getenv(
                    "AUTOPILOT_BEAT_MAX_TOKENS_RATIO",
                    str(DEFAULT_AUTOPILOT_BEAT_MAX_TOKENS_RATIO),
                )
            ),
        )
        self.autopilot_enable_beats = _env_flag("AUTOPILOT_ENABLE_BEATS", True)
    ...
    def _save_only_if_db_still_running(self, novel: Novel) -> bool:
        """原子保存：仅当 DB 当前仍是 RUNNING 时才写回。

        这能消除「先读 RUNNING，用户随后点 STOPPED，再被守护进程 save 覆盖回 RUNNING」的竞态。
        """
        saver = getattr(self.novel_repository, "save_if_autopilot_status", None)
        if not callable(saver):
            return False

        saved = bool(saver(novel, AutopilotStatus.RUNNING.value))
        if saved:
            return True

        self._merge_autopilot_status_from_db(novel)
        logger.info(
            "[%s] 检测到外部状态已变更为 %s，跳过本次写库",
            novel.novel_id,
    ...
    def _flush_novel(self, novel: Novel, merge_db_status: bool = True) -> None:
        """关键阶段立即写库，避免下一轮轮询仍读到旧 stage（重复幕级规划 / 重复日志）。"""
        if merge_db_status and self._save_only_if_db_still_running(novel):
            return
        if merge_db_status:
            self._merge_autopilot_status_from_db(novel)
        self.novel_repository.save(novel)

    def _save_novel_state(self, novel: Novel, merge_db_status: bool = True) -> None:
    ...
    async def _process_novel(self, novel: Novel):
        ...
        try:
            ...
            # 短文模式:章节节点已由 NovelService.create_article_novel 预埋,
            # 不需要宏观/幕级规划,亦跳过审计。强制对齐到 WRITING。
            if getattr(novel, "length_tier", "") == "article":
                if novel.current_stage in (
                    NovelStage.PLANNING,
                    NovelStage.MACRO_PLANNING,
                    NovelStage.ACT_PLANNING,
                    NovelStage.AUDITING,
                ):
                    logger.info(f"[{novel.novel_id}] 🪶 短文模式:跳过 {novel.current_stage.value} 直接进入 WRITING")
            ...
            self._save_novel_state(
                novel,
                merge_db_status=novel.autopilot_status != AutopilotStatus.ERROR,
            )
    ...
    async def _handle_act_planning(self, novel: Novel):
        ...
        novel_id = novel.novel_id.value
        target_act_number = (novel.current_act or 0) + 1  # 1-indexed
        ...
        if act_nodes:
            max_existing_act_number = max((n.number or 0) for n in act_nodes)
            next_act_number = max_existing_act_number + 1
            if target_act_number > next_act_number:
                logger.warning(
                    f"[{novel.novel_id}] current_act 指针漂移：请求第 {target_act_number} 幕，"
                    f"实际最后一幕为第 {max_existing_act_number} 幕；已回正到第 {next_act_number} 幕"
                )
                target_act_number = next_act_number
                novel.current_act = max_existing_act_number
            ...
            try:
                ...
                if last_act:
                    ...
                    volume_nodes = sorted(
                        ...
                        key=lambda n: ((n.number or 0), getattr(n, "order_index", 0) or 0)
                    )
                    parent_volume = volume_nodes[0] if volume_nodes else None
                    if parent_volume:
                        logger.info(f"[{novel.novel_id}] 创建首幕（父卷：第 {parent_volume.number} 卷）")
                        ...
                        first_act = StoryNode(
                            ...
                            suggested_chapter_count=max((novel.target_chapters or 30) // 3, 5),
                        )
    ...
    async def _handle_writing(self, novel: Novel):
        ...
        if self.chapter_workflow:
            try:
                bundle = self.chapter_workflow.prepare_chapter_generation(
                    ...
                    outline,
                    scene_director=None,
                    max_tokens=self.autopilot_context_max_tokens,
                )
        ...
        # 5. 节拍放大(短文模式强制跳过:1 章 1 万字 1 次 LLM 调用一气呵成)
        is_article = getattr(novel, "length_tier", "") == "article"
        beats = []
        if self.context_builder and self.autopilot_enable_beats and not is_article:
            tw = getattr(novel, "target_words_per_chapter", None) or self.autopilot_target_chapter_words
            beats = self.context_builder.magnify_outline_to_beats(
                chapter_num,
                outline,
                target_chapter_words=int(tw),
            )
        ...
        if beats:
            for i, beat in enumerate(beats):
                ...
                if use_wf:
                    ...
                    max_tokens = max(
                        800,
                        int(beat.target_words * self.autopilot_beat_max_tokens_ratio),
                    )
            ...
            if use_wf:
                ...
                if is_article:
                    # 1 章 1 万字 1 次 LLM 调用:放开 max_tokens 容纳整篇;
                    # 模型沿用 provider 默认(Anthropic 直连/OpenAI 兼容/火山方舟 ARK),
                    # 避免硬写模型 ID 与当前 provider 不兼容导致连接失败。
                    # 想切到更快的小模型时,在 .env 改 OPENAI_MODEL / ANTHROPIC_MODEL 即可。
                    cfg = GenerationConfig(max_tokens=16000, temperature=0.85)
                    # 预落一个空 draft,让 chapter-stream SSE 端点立刻检测到"有 draft 在写",
                    # 从而向前端发出 chapter_start 事件、激活流式条。否则单次 LLM 调用 60s
                    # 全程只在结束后才 upsert,前端整段时间看不到文字。
                    await self._upsert_chapter_content(novel, next_chapter_node, "", status="draft")
                else:
        ...
        if not chapter_content.strip():
            raise RuntimeError(f"LLM returned empty content for chapter {chapter_num}")
        ...
        novel.current_beat_index = 0
        # 短文模式跳过审计:直接回到 WRITING(下一轮要么继续下一章,要么命中 target 进入 COMPLETED)
        if getattr(novel, "length_tier", "") == "article":
            novel.current_stage = NovelStage.WRITING
        else:
            novel.current_stage = NovelStage.AUDITING
...
4. 节奏控制：不要一章推进太多剧情
5. 不要写章节标题
6. 每次输出收在完整句群或完整自然段，不要断字断句，段末保留悬念"""
    ...
    def _get_protagonist_name(self, novel_id: str) -> Optional[str]:
        workflow = self.chapter_workflow
        bible_repository = getattr(workflow, "bible_repository", None) if workflow else None
        ...
        try:
            ...
            for character in bible.characters:
                ...
                desc = str(getattr(character, "description", "") or "")
                char_id = str(getattr(character, "id", "") or "")
                if not name:
                    continue
                if char_id.endswith("char-1") or "主角" in desc or "主人公" in desc:
                    return name
            ...
            logger.debug("获取主角名称失败: %s", e)

        return None

    def _normalize_generated_content(self, novel_id: str, content: str) -> str:
        text = content or ""
        if "主角" not in text:
            return text

        protagonist_name = self._get_protagonist_name(novel_id)
        if not protagonist_name:
            return text

        return text.replace("主角", protagonist_name)
    ...
    async def _upsert_chapter_content(self, novel, chapter_node, content: str, status: str):
        ...
        content = self._normalize_generated_content(novel.novel_id.value, content)
