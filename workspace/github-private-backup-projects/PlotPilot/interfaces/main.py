...
app = FastAPI(
    title="小说创作家 API",
    version="1.0.1",
    description="小说创作家 AI 小说创作平台 API",
...
if _FRONTEND_DIR.exists():
    ...
    _architecture = _FRONTEND_DIR / "architecture.html"
    if _favicon.exists():
    ...
    if _architecture.exists():
        app.get("/architecture.html", include_in_schema=False, response_class=FileResponse)(
            lambda: FileResponse(str(_architecture), media_type="text/html")
        )
...
async def root():
    ...
    return {"message": "小说创作家 API", "release": APP_RELEASE_VERSION}
