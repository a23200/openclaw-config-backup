...
class TestBuildPrompt:
    ...
    def test_build_prompt_requires_complete_paragraph_and_suspense(self, workflow):
        """提示词应明确要求完整收束，避免断字断句。"""
        prompt = workflow._build_prompt(
            context="CTX",
            outline="OL",
            ...
            beat_index=0,
            total_beats=3,
            beat_target_words=800,
        )

        assert "不要断字、断词、断半句" in prompt.system
        assert "段落收束必须完整自然" in prompt.user
        assert "结尾要有悬念或转折" in prompt.user
...
class TestStyleIntegration:
    ...
    async def test_generate_chapter_injects_fingerprint_summary(
        ...
        prompts = [
            ...
            if call.kwargs.get("prompt") or call.args
        ]

        assert any(
            "形容词密度" in prompt.system or "平均句长" in prompt.system
