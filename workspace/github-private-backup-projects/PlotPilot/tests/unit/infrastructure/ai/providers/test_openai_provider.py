...
class TestOpenAIProviderResponses:
    ...
    @pytest.mark.anyio
    async def test_generate_empty_responses_falls_back_to_chat(self, provider):
        prompt = Prompt(system="You are helpful", user="Hello")
        config = GenerationConfig(model="test-model")
        responses_empty = SimpleNamespace(
            output=[],
        ...
        with (
            patch.object(provider.async_client.responses, "create", new_callable=AsyncMock) as mock_responses,
            patch.object(provider.async_client.chat.completions, "create", new_callable=AsyncMock) as mock_chat,
        ):
            mock_responses.return_value = responses_empty
    ...
    @pytest.mark.anyio
    async def test_stream_empty_responses_falls_back_to_chat_stream(self, provider):
        prompt = Prompt(system="You are helpful", user="Hello")
        config = GenerationConfig(model="gpt-5.4", temperature=0.7, max_tokens=32)
        responses_stream = _FakeStream([SimpleNamespace(type="response.completed")])
