...
class SqliteNovelRepository(NovelRepository):
    ...
    def _build_save_sql(self, expected_current_autopilot_status: Optional[str] = None) -> str:
        sql = """
            INSERT INTO novels (
                ...
                target_words_per_chapter, length_tier,
        ...
        """
        if expected_current_autopilot_status is not None:
            sql += "\n            WHERE novels.autopilot_status = ?"
        return sql

    def _build_save_params(self, novel: Novel) -> tuple:
        ...
        length_tier = getattr(novel, "length_tier", "") or ""
    ...
    def _save_with_expected_status(
        self,
        novel: Novel,
        expected_current_autopilot_status: Optional[str] = None,
    ) -> int:
        sql = self._build_save_sql(expected_current_autopilot_status)
        params = self._build_save_params(novel)
        if expected_current_autopilot_status is not None:
            params = params + (expected_current_autopilot_status,)
    ...
    def save(self, novel: Novel) -> None:
        """保存小说"""
        self._save_with_expected_status(novel)

    def save_if_autopilot_status(self, novel: Novel, expected_status: str) -> bool:
        """仅当数据库当前 autopilot_status 匹配 expected_status 时保存。

        用于守护进程写回阶段状态时，避免把用户刚点下的 STOPPED 再覆盖回 RUNNING。
        """
        return self._save_with_expected_status(novel, expected_status) > 0
