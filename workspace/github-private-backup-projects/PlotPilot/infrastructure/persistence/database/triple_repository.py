...
class TripleRepository:
    ...
    @staticmethod
    def _entity_aliases(novel_id: str, entity_id: Optional[str]) -> list[str]:
    ...
    def _load_entity_label_map(self, novel_id: str) -> dict[str, str]:
        labels: dict[str, str] = {}
        for table_name in ("bible_characters", "bible_locations"):
            rows = self._db.fetch_all(
                f"SELECT id, name FROM {table_name} WHERE novel_id = ?",
                (novel_id,),
            )
            for row in rows:
                ...
                if not entity_id or not name:
                    continue
                for alias in self._entity_aliases(novel_id, entity_id):
                    labels[alias] = name
        return labels

    def _resolve_entity_label(self, novel_id: str, entity_id: Optional[str]) -> Optional[str]:
        entity_labels = self._load_entity_label_map(novel_id)
        for alias in self._entity_aliases(novel_id, entity_id):
            label = entity_labels.get(alias)
            if label:
                return label
        return None

    def _with_display_labels(self, triple: Triple) -> Triple:
        attrs = dict(triple.attributes or {})
        subject_label = str(attrs.get("subject_label") or "").strip()
        object_label = str(attrs.get("object_label") or "").strip()

        if not subject_label:
            resolved = self._resolve_entity_label(triple.novel_id, triple.subject_id)
            if resolved:
                attrs["subject_label"] = resolved

        if not object_label:
            resolved = self._resolve_entity_label(triple.novel_id, triple.object_id)
            if resolved:
                attrs["object_label"] = resolved
    ...
    def persist_triple_sync(self, novel_id: str, triple: Triple) -> None:
        ...
        triple.novel_id = novel_id
        self._kr.save_triple(novel_id, _triple_to_fact_dict(self._with_display_labels(triple)))
    ...
    async def save(self, triple: Triple) -> Triple:
        self._kr.save_triple(triple.novel_id, _triple_to_fact_dict(self._with_display_labels(triple)))
        return triple
    ...
    async def save_with_provenance(
        ...
        self._kr.save_triple(
            triple.novel_id,
            _triple_to_fact_dict(self._with_display_labels(triple)),
