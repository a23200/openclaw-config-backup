...
class SqliteKnowledgeRepository:
    ...
    def _load_entity_label_map(self, novel_id: str) -> Dict[str, str]:
        ...
        for row in self.db.fetch_all(
            "SELECT id, name FROM bible_characters WHERE novel_id = ?",
            (novel_id,),
        ):
            entity_id = row["id"]
        ...
        for row in self.db.fetch_all(
            "SELECT id, name FROM bible_locations WHERE novel_id = ?",
            (novel_id,),
        ):
            entity_id = row["id"]
            name = str(row["name"] or "").strip()
    ...
    def _resolve_entity_label(
        ...
        resolved = ""
        for alias in self._entity_aliases(novel_id, entity):
            resolved = entity_labels.get(alias, "")
            if resolved:
                break
        ...
        should_replace_raw = (
            not raw
            or raw == entity
            or raw in GENERIC_CHARACTER_LABELS
            or raw.startswith(f"{novel_id}-char-")
            or raw.startswith(f"{novel_id}-loc-")
            or raw.startswith("char-")
            or raw.startswith("loc-")
        )

        if resolved and should_replace_raw:
            return resolved
        if raw:
            return raw
        if resolved:
    ...
    def _build_facts_from_triple_rows(
        ...
        entity_labels = self._load_entity_label_map(novel_id_str)
        ...
        for row in triples_rows:
            ...
            facts.append(
                KnowledgeTriple(
                    id=tid,
                    subject=self._resolve_entity_label(
                        novel_id_str,
                        row["subject"],
                        subject_entity_id,
                        entity_labels,
                    ...
                    object=self._resolve_entity_label(
                        novel_id_str,
                        row["object"],
                        object_entity_id,
                        entity_labels,
