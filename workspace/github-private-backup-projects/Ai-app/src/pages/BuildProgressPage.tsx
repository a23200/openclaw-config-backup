...
export default function BuildProgressPage() {
  ...
  const [persistedBuildLog, setPersistedBuildLog] = useState("");
  const [logView, setLogView] = useState<"events" | "buildLog">("events");
  ...
  }, [buildStatus?.logs, persistedBuildLog, logView]);
  ...
  useEffect(() => {
    ...
    async function syncBuildLog() {
      try {
        const nextLog = await getBuildLog(id);
        if (!cancelled) {
          setPersistedBuildLog(nextLog.content);
        }
      } catch {
        if (!cancelled) {
          setPersistedBuildLog("");
        }
      }
    }
    ...
    if (!["ready", "failed"].includes(currentStatus)) {
  ...
  }, [id, currentStatus, streamVersion]);

  useEffect(() => {
    if (persistedBuildLog.trim() && ["ready", "failed"].includes(currentStatus)) {
      setLogView("buildLog");
    }
  }, [currentStatus, persistedBuildLog]);
  ...
  const progressTarget = useMemo(() => {
    ...
    if (buildStatus.status === "failed") {
      ...
      if (buildStatus.step === "codegen") {
        return Math.max(42, Math.min(84, 32 + files * 3.4));
      }
    ...
    switch (buildStatus.step) {
      ...
      case "codegen":
        return Math.min(
          82,
          28 + files * 3.4 + (buildStatus.streamState === "streaming" ? 8 : 0),
        );
      case "compile":
  ...
  const terminalLines = useMemo(() => {
    if (logView === "buildLog" && persistedBuildLog.trim()) {
      return persistedBuildLog.replace(/\r/g, "").split("\n").filter((_, index, lines) => {
        return !(index === lines.length - 1 && lines[index] === "");
      });
    }

    return buildStatus?.logs ?? [];
  }, [buildStatus?.logs, logView, persistedBuildLog]);

  function handleDownloadBuildLog() {
    const content =
      persistedBuildLog.trim() ||
      (buildStatus?.logs?.length ? `${buildStatus.logs.join("\n")}\n` : "");
    if (!content) {
      return;
    }
  ...
  async function handleRecompileOnly() {
    setRecompiling(true);
    setError("");
    try {
      await startBuild(id, { skipCodegen: true });
      setLoading(true);
      setBuildStatus(null);
      setTransport("sse");
      setStreamVersion((value) => value + 1);
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : "重新编译失败");
    } finally {
              ...
              {buildStatus?.status === "failed" ? (
                <div className="space-y-2">
                  <Button
                    ...
                    onClick={handleRecompileOnly}
                    disabled={recompiling || restarting}
                  >
                    <span>{recompiling ? "正在重新编译..." : "仅重新编译（保留已生成代码）"}</span>
                  ...
                  <Button
                    ...
                    disabled={restarting || recompiling}
                  >
                    <span>{restarting ? "正在重新启动..." : "重新生成并构建"}</span>
                    <RefreshCw className="h-4 w-4" />
                  </Button>
                </div>
              ) : null}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex items-center rounded-full border border-white/10 bg-white/5 p-1">
                      ...
                      <Button
                        type="button"
                        variant={logView === "buildLog" ? "secondary" : "ghost"}
                        size="sm"
                        className="h-8 rounded-full px-3 text-xs"
                        onClick={() => setLogView("buildLog")}
                        disabled={!persistedBuildLog.trim()}
                      >
                        完整 build.log
                      </Button>
                    </div>
                    <Button
                      ...
                      size="sm"
                      className="gap-1.5 px-3.5 py-1.5"
                      onClick={handleDownloadBuildLog}
                      disabled={!persistedBuildLog.trim() && !buildStatus?.logs?.length}
                    >
                ...
                <div className="terminal-surface overflow-hidden">
                  <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
                    ...
                    <p className="text-xs uppercase tracking-[0.24em] text-slate-500">
                      {logView === "buildLog" ? "Full build.log" : "Build terminal"}
                    </p>
                  </div>
                  <ScrollArea className="h-[320px] text-slate-100">
                    ...
                    >
                      ...
                      {terminalLines.length ? (
                        terminalLines.map((logLine, index) => (
                          <div
                            key={`${logLine}-${index}`}
