...
function getSessionId(sessionKey) {
  return new Promise((resolve, reject) => {
    if (sessionIdCache[sessionKey]) {
      return resolve(sessionIdCache[sessionKey]);
    }

    console.log(`[Agent] Looking up session ID for key: ${sessionKey}`);
    exec('openclaw sessions --json', (error, stdout, stderr) => {
      if (error) {
        console.error(`[Agent] Error looking up session ID: ${error.message}`);
        return reject(error);
      }
      try {
        // Clean up stdout from potential warnings/logs before JSON parsing
        const jsonStartIndex = stdout.indexOf('{');
        const jsonOutput = stdout.substring(jsonStartIndex);
        const data = JSON.parse(jsonOutput);
        const session = data.sessions.find(s => s.key === sessionKey);
        if (session && session.sessionId) {
          console.log(`[Agent] Found session ID: ${session.sessionId}`);
          sessionIdCache[sessionKey] = session.sessionId;
          resolve(session.sessionId);
        } else {
          reject(new Error(`Session with key "${sessionKey}" not found.`));
        }
      } catch (parseError) {
        console.error(`[Agent] Error parsing session list: ${parseError}`);
        console.error(`[Agent] Raw stdout was:\n${stdout}`);
        reject(parseError);
      }
    });
  });
...
// --- MCP Server Logic ---
function createMcpServer(agentSessionKey) {
  const tools = {
    'chat': {
      description: "Send a message to Laodi and get a response.",
      inputSchema: { // Corrected from 'args' to 'inputSchema' for xiaozhi protocol
        message: { type: "string", description: "The user's message." }
      },
      handler: async (args) => {
        console.log(`[MCP Server] Received chat message: "${args.message}"`);
        try {
          const sessionId = await getSessionId(agentSessionKey);
          return new Promise((resolve, reject) => {
            const command = `openclaw agent --session-id "${sessionId}" --message "${args.message.replace(/"/g, '\\"')}"`;
            ...
            console.error('[MCP Server] Failed to get session ID for chat handler.', error);
            return { error: { message: "Could not find the agent session to send the message to." } };
        }
      }
    }
  };
  ...
  async function handleRequest(request) {
    ...
    switch (method) {
      case 'tools/list':
        return { id, result: { tools: Object.fromEntries(Object.entries(tools).map(([k, v]) => [k, {description: v.description, inputSchema: v.inputSchema}])) }};
      
      case 'tools/call':
        const tool = tools[params.name];
        if (tool) {
          const result = await tool.handler(params.input);
          return { id, result };
        } else {
          return { id, error: { message: `Tool not found: ${params.name}` } };
        }
      ...
      default:
        return { id, error: { message: `Method not found: ${method}` } };
    }
  }
