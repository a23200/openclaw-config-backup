...
- **Douyin Publishing (2026-03-21)**: 绝对不能把“图文”和“文章”发布搞混。
  - **发图文 (Image-Text)**: 必须使用 `node ~/.agents/skills/douyin-creator-tools/src/publish-imagetext.mjs`。JSON 载荷必须是 `description` 和 `imagePaths` (数组)。
  - **发文章 (Article)**: 才是 `publish-douyin-article.mjs`。不要默认调用这个。
