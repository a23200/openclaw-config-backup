...
def sync_conversation_messages_best_effort(
    ...
    if str(os.getenv('YUYU_ENABLE_LOCAL_BROWSER_CHAT_SYNC', '')).strip().lower() not in {'1', 'true', 'yes', 'on'}:
        logger.info(f"【{normalized_cookie_id}】已禁用聊天页本机浏览器补同步，避免非验证码场景弹出调用申请")
        return {
            'success': False,
            'skipped': 'local_browser_chat_sync_disabled',
            'error': '已禁用本机浏览器聊天补同步',
        }
