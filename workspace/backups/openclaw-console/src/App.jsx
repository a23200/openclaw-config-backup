...
import { fetchJsonOrThrow } from "./lib/http.js";
import { extractSessionKey } from "./lib/openclaw.js";

const ClawLinkOffice3D = React.lazy(() => import("./components/ClawLinkOffice3D.jsx"));
        ...
        <div className="session-drawer-list">
          {sessions.length ? (
            ...
            <div className="placeholder-card">
              {mode === "mock" ? "演示模式下不拉取真实会话。" : "暂无会话数据，先检查连接或创建会话。"}
            </div>
          )}
        </div>
      </aside>
...
export default function App() {
  ...
  const connectionState = mode === "mock" ? "演示模式" : connectionInfo ? "已连接" : "待检查";
  const connectionTone = mode === "mock" ? "ok" : connectionInfo ? "ok" : "warn";
  ...
  const topbarRuntimeState = mode === "mock" ? "mock" : connectionInfo ? "live" : "waiting";
  const topbarRuntimeLabel = mode === "mock" ? "Mock" : connectionInfo ? "Live" : "Waiting";
    ...
    () => [
      {
        ...
        emoji: "⛩",
        color: "#ff6b5e",
        status: mode === "mock" ? "idle" : connectionInfo ? "working" : "thinking",
        statusLabel: connectionState,
        note: connectionInfo?.gatewayBaseUrl || "等待网关探测",
      },
      {
        id: "chat",
        kind: "system",
  ...
  async function handleCheckHealth() {
    try {
      ...
      pushSystemMessage("已成功连接到 ClawLink 后端。");
      pushActivityLog("健康检查成功", data.gatewayBaseUrl || "后端已在线", "ok");
    } catch (error) {
      pushSystemMessage(`连接检查失败：${error.message}`);
  ...
  React.useEffect(() => {
    if (mode !== "live" || activeView !== "chat" || !sessionKey) return undefined;
  ...
  }, [activeView, historyEndpoint, mode, sessionKey]);
      ...
      <div className="overview-workspace">
        <section className="hero-panel overview-hero-panel">
          <div className="overview-hero-layout">
            ...
            <div className="overview-hero-side">
              <StatusBadge
                label={BRANDING.gatewayName}
                value={mode === "mock" ? "模拟运行" : connectionInfo ? "运行正常" : "等待检查"}
                tone={connectionTone}
                pulse={Boolean(connectionInfo) || mode === "mock"}
              />
        ...
        <div className="overview-metrics-grid">
          <MetricCard
            ...
            hint={mode === "mock" ? "当前界面处于演示模式。" : connectionInfo?.gatewayBaseUrl || "等待健康检查。"}
            icon={PlugZap}
            tone={connectionTone}
          />
          <MetricCard
            ...
            <div className="overview-context-stack">
              <div className="overview-context-card highlight">
                ...
                <div className="overview-context-text">
                  {currentSession?.lastMessage || "可以先检查连接、创建会话，再切到聊天页继续操作。"}
                </div>
              </div>
          ...
          <div className="chat-main-header">
            <div className="chat-header-copy min-w-0">
              ...
              <div className="chat-view-meta text-xs text-[var(--text-secondary)]">
                ...
                <span>·</span>
                <span>{mode === "mock" ? "演示模式" : "2.5s 自动同步"}</span>
              </div>
            </div>
            ...
            <div className="chat-editor">
              ...
              <div className="chat-editor-footer">
                <div className="flex flex-wrap items-center gap-2 text-xs text-[var(--text-secondary)]">
                  <span className={`live-dot ${connectionInfo ? "online" : ""}`} />
                  <span>{mode === "mock" ? "演示模式" : connectionInfo ? "Gateway 已连接" : "等待连接检查"}</span>
                  <span>·</span>
        ...
        <div className="chat-side-column">
          <SectionCard title="连接概览" description="借鉴 dashboard 的状态卡片组织方式。" icon={PlugZap}>
            <div className="space-y-3">
              ...
              <StatusBadge label="连接状态" value={connectionState} tone={connectionTone} pulse={Boolean(connectionInfo)} />
          ...
          >
            <div className="space-y-3">
              {sessions.length ? (
                ...
                <div className="placeholder-card">当前还没有会话数据，先检查连接或创建会话。</div>
              )}
            </div>
          </SectionCard>
            ...
            <SectionCard title="最近后端日志" description="这里已经是后端真实活动数据，不再只是前端占位。" icon={Activity}>
              <div className="space-y-3">
                {serverLogs.length ? (
                  ...
                  <div className="placeholder-card">后端日志为空，执行一次检查连接或刷新会话后这里会出现记录。</div>
                )}
              </div>
            </SectionCard>
          </div>
      ...
      <div className="scene-workspace">
        ...
        <section className="scene-stage-panel app-panel">
          <div className="scene-stage-header">
            <div className="min-w-0">
              <div className="topbar-eyebrow">3D Office</div>
              <h2 className="scene-stage-title text-[var(--text-primary)]">ClawLink 控制层</h2>
              <div className="scene-stage-meta text-xs text-[var(--text-secondary)]">
                <span>{sceneAgents.length} 个 3D 工位</span>
                <span>·</span>
                <span>{sceneAgentCount} 个 Agent</span>
                <span>·</span>
            ...
            <div className="scene-preset-row">
              {SCENE_PRESET_OPTIONS.map((preset) => (
                <button
                  key={preset.id}
                  type="button"
                  className={`scene-preset-chip ${sceneCameraPreset === preset.id ? "active" : ""}`}
                  onClick={() => handleApplyScenePreset(preset.id, preset.focusId)}
                >
                  {preset.label}
                </button>
              ))}
          ...
          <div className="scene-canvas-shell">
            <React.Suspense fallback={<div className="scene-loading-state">正在加载 3D 场景…</div>}>
              <ClawLinkOffice3D
                ...
                highlightId={sceneFocusId}
                summary={sceneSummary}
                onSelect={handleFocusSceneModule}
              />
            </React.Suspense>
          </div>
        </section>
          ...
          >
            <div className="space-y-3">
              {serverLogs.length ? (
                ...
                <div className="placeholder-card">后端日志暂时为空，执行一次检查连接、拉取会话或发送消息后，这里会出现真实记录。</div>
              )}
            </div>
          </SectionCard>
