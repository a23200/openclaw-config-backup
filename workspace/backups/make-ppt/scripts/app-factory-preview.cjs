...
const providerCatalog = {
  ...
  plus_ai: {
    ...
    defaultTemplateKey: 'modernist_professional',
  },
};
...
async function prepareSourceMaterial(brief) {
  const runs = [];
...
async function createPlusAiProject(brief, projectSlug) {
  const runs = [];
  const sourceMaterial = await prepareSourceMaterial(brief);
...
async function createProject(rawPayload) {
  ...
  const templateMeta = provider.templates[payload.templateKey] || {};
  ...
  const pipeline = provider.key === 'plus_ai'
    ? await createPlusAiProject(brief, projectSlug)
    : await createPptMasterProject(brief, projectSlug);
  const pipelineRunsPath = path.join(projectDir, `${provider.key}-runs.json`);
  await fsp.writeFile(pipelineRunsPath, JSON.stringify(pipeline.runs, null, 2), 'utf8');

  brief.pptMasterRoot = pipeline.pptMasterRoot || '';
  brief.pptSkillDir = pipeline.pptSkillDir || '';
  brief.pptPython = pipeline.pptPython || '';
  brief.pptMasterProjectPath = pipeline.pptProjectPath || '';
  brief.pptMasterDesignSpecPath = pipeline.pptDesignSpecPath || '';
  brief.markdownPath = pipeline.markdownPath || '';
  brief.providerRunMetadataPath = pipeline.metadataPath || '';
  brief.providerExternalId = pipeline.externalId || '';
  brief.pipelineRunsPath = pipelineRunsPath;
  const resolvedArtifacts = await resolveProjectArtifactPaths({
    ...brief,
    pptMasterProjectPath: brief.pptMasterProjectPath,
    markdownPath: brief.markdownPath,
  });
  brief.exportPptxPath = resolvedArtifacts.exportPptxPath || pipeline.exportPptxPath || brief.exportPptxPath;
  brief.exportSvgPptxPath = resolvedArtifacts.exportSvgPptxPath || pipeline.exportSvgPptxPath || brief.exportSvgPptxPath;
  brief.markdownPath = resolvedArtifacts.markdownPath || brief.markdownPath;
  brief.status = (pipeline.runs || []).every((run) => run.ok)
    ? `${provider.key}_ready`
    : `${provider.key}_partial`;
  brief.resultDocPath = await writeProjectHandoff(brief, pipeline);
